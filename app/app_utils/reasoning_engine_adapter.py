...
def _normalize_kwargs(kwargs: dict) -> dict:
    """Normalize input kwargs sent by Vertex AI Console Playground or SDK calls."""
    if not isinstance(kwargs, dict):
        kwargs = {"message": str(kwargs)}
    else:
        kwargs = dict(kwargs)

    if "message" not in kwargs:
        if "input" in kwargs:
            kwargs["message"] = kwargs.pop("input")
        elif "prompt" in kwargs:
            kwargs["message"] = kwargs.pop("prompt")
        elif "text" in kwargs:
            kwargs["message"] = kwargs.pop("text")
        elif "query" in kwargs:
            kwargs["message"] = kwargs.pop("query")
        else:
            kwargs["message"] = ""

    if "user_id" not in kwargs or not kwargs["user_id"]:
        kwargs["user_id"] = "console_user"
...
def attach_reasoning_engine_routes(app: FastAPI) -> None:
    ...
    def resolve_method(class_method: str, *, streaming: bool):
        ...
        if class_method in ("query", "async_query", "stream_query", "async_stream_query"):
            target_method = "async_stream_query" if streaming else "stream_query"
            if hasattr(rt, target_method):
                return getattr(rt, target_method)
            if hasattr(rt, "async_stream_query"):
                return getattr(rt, "async_stream_query")
        if hasattr(rt, class_method):
    ...
    async def stream_query(request: Request) -> responses.StreamingResponse:
        ...
        method = resolve_method(body.get("class_method", "async_stream_query"), streaming=True)
        kwargs = _normalize_kwargs(body.get("input") or {})
        stream = (
            await method(**kwargs)
    ...
    async def query(request: Request) -> responses.JSONResponse:
        body = await request.json()
        method = resolve_method(body.get("class_method", "query"), streaming=False)
        kwargs = _normalize_kwargs(body.get("input") or {})
        if inspect.iscoroutinefunction(method):
